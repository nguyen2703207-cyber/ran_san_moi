...
class MAIN:
    ...
    def check_fail(self):
        global game_state
        # Chỉ kiểm tra thua nếu rắn đã bắt đầu di chuyển
    ...
    def draw_gameover_screen(self):
        ...
        screen.blit(over_text, over_text.get_rect(center=(cell_size*cell_number//2, cell_size*cell_number//2 - 50)))
        screen.blit(restart_text, restart_text.get_rect(center=(cell_size*cell_number//2, cell_size*cell_number//2 + 50)))
...
while True:
        ...
        if game_state == "start":
            if event.type == pygame.MOUSEBUTTONDOWN:
                if play_button.collidepoint(event.pos):
                    ...
                    main_game.snake.direction = Vector2(-1,0)  # hướng mặc định
                    game_state = "playing"
    ...
    # Vẽ theo trạng thái
    if game_state == "start":
        play_button = main_game.draw_start_screen()
    elif game_state == "playing":
        screen.fill((175,215,70))
        main_game.draw_element()
    elif game_state == "gameover":
        main_game.draw_gameover_screen()
